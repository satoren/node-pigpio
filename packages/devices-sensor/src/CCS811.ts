  ...
  const ccs811 = new (class extends EventEmitter {
    ...
    async checkForError(status?: number, errorId?: number) {
      ...
      if (status & (1 << 0)) {
        ...
        if (errorId & (1 << 5)) {
          ...
          errorMessage =
            'MAX_RESISTANCE: The MOX material is no longer functioning as expected.'
        } else if (errorId & (1 << 2)) {
          errorMessage =
            'MEASMODE_INVALID: The CCS811 received an I2C request to write an unsupported mode to MEAS_MODE '
        } else if (errorId & (1 << 1)) {
          errorMessage =
            'READ_REG_INVALID: The CCS811 received an I2C read request to a mailbox ID that is invalid'
        } else if (errorId & (1 << 0)) {
          errorMessage =
            'MSG_INVALID: The CCS811 received an I2C write request addressed to this station but with invalid mailbox ID or the wrong size '
        } else {
    ...
    async measure() {
      const [
        ...
        status,
        errorId,
      ] = await readData(RegisterAddress.ALG_RESULT_DATA, 8)
      if (status & (1 << 3)) {
        const old = { CO2, tVOC }
