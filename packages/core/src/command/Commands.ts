...
type P1name<T extends RequestTypeNames> = ParamType<RequestCommandType[T]['p1']>
type P2name<T extends RequestTypeNames> = ParamType<RequestCommandType[T]['p2']>
...
type ExtensionRequestCommand<T extends RequestTypeNames> = {
  command: T
} & ExtensionRequestCommandP1<T> &
  ExtensionRequestCommandP2<T>
...
export const createRequestParam = <T extends RequestTypeNames>(
  ...
  const responseExtension =
    responseExtensionName[0] !== '-' && responseExtensionName[0] !== 'N/A'
  return {
    cmd,
    p1,
    p2,
