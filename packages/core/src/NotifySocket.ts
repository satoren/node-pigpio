...
class SocketImpl extends EventEmitter implements NotifySocket {
  ...
  constructor(private control: RequestSocket) {
    super()
  ...
  async start() {
    ...
    this.lastLevel = (
      await requestFactory(sock, createRequestParam({ command: 'BR1' }))()
    ).res
    this.handle = (
  ...
  appendEvent(e: EventCallback): void {
    ...
    void this.control.request(
      createRequestParam({ command: 'EVM', bits, handle })
    )
  }
  ...
  removeEvent(e: EventCallback): void {
    ...
    this.eventBits = Array(...this.events.values()).reduce(
      (prev, cur) => prev | cur.bit,
      0
    )
    const bits = this.eventBits
    void this.control.request(
  ...
  append(e: EdgeCallback): void {
    ...
    void this.control.request(
      createRequestParam({ command: 'NB', bits, handle })
    )
  }
  ...
  remove(e: EdgeCallback): void {
    ...
    this.monitor = Array(...this.callbacks.values()).reduce(
      (prev, cur) => prev | cur.bit,
      0
    )
  ...
  async close(): Promise<void> {
    ...
    if (handle != null) {
      await requestFactory(
        sock,
        createRequestParam({ command: 'NC', handle })
      )()
    }
    this.sock.destroy()
  }
