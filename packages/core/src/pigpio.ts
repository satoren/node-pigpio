...
type ScriptStatusType =
  | typeof PI_SCRIPT_INITING
  | typeof PI_SCRIPT_HALTED
  | typeof PI_SCRIPT_RUNNING
  | typeof PI_SCRIPT_WAITING
  | typeof PI_SCRIPT_FAILED
...
export async function pi(host?: string, port?: number): Promise<pigpio> {
  ...
  class PiImpl implements pigpio {
    ...
    set_watchdog(user_gpio: number, wdog_timeout: number): Promise<number> {
      return pigpioCommand(
    ...
    custom_1(arg1 = 0, arg2 = 0, argx = Buffer.of()): Promise<number> {
      return pigpioCommand(
    ...
    store_script(script: string): Promise<number> {
      return pigpioCommand(
        reqParam({ command: 'PROC', extension: Buffer.from(script, 'utf-8') })
      )
    }
    ...
    async script_status(
      script_id: number
    ): Promise<[ScriptStatusType, number[]]> {
      const [res, d] = await pigpioCommandExt(
        reqParam({ command: 'PROCP', script_id })
      )
    ...
    i2c_write_byte(handle: number, byte_val: number): Promise<number> {
      return pigpioCommand(
        reqParam({ command: 'I2CWS', handle, byte: byte_val })
      )
    }

    i2c_read_byte_data(handle: number, reg: number): Promise<number> {
      return pigpioCommand(
        reqParam({ command: 'I2CRB', handle, register: reg })
      )
    }
    ...
    i2c_write_byte_data(
      ...
      return pigpioCommand(
        reqParam({ command: 'I2CWB', handle, register: reg, extension })
      )
    }

    i2c_read_word_data(handle: number, reg: number): Promise<number> {
      return pigpioCommand(
        reqParam({ command: 'I2CRW', handle, register: reg })
      )
    }
    ...
    i2c_write_word_data(
      ...
      return pigpioCommand(
        reqParam({ command: 'I2CWW', handle, register: reg, extension })
      )
    }
    ...
    i2c_read_i2c_block_data(
      ...
      return pigpioCommandExt(
        reqParam({ command: 'I2CRI', handle, register: reg, extension })
      )
    }
    ...
    i2c_process_call(
      ...
      extension.writeUInt32LE(word_val)
      return pigpioCommand(
        reqParam({ command: 'I2CPC', handle, register: reg, extension })
      )
    }
    ...
    bb_i2c_open(SDA: number, SCL: number, baud: number): Promise<number> {
      ...
      return pigpioCommand(
        reqParam({ command: 'BI2CO', sda: SDA, scl: SCL, extension })
      )
    }
    ...
    serial_open(tty: string, baud: number, ser_flags = 0): Promise<number> {
      return pigpioCommand(
        reqParam({
          command: 'SERO',
          baud,
    ...
    serial_write_byte(handle: number, byte_val: number): Promise<number> {
      return pigpioCommand(
        reqParam({ command: 'SERWB', handle, byte: byte_val })
      )
    }
    ...
    serial_write(handle: number, data: Buffer): Promise<number> {
      return pigpioCommand(
        reqParam({ command: 'SERW', handle, extension: data })
      )
    }
    ...
    bb_serial_read_open(
      ...
      return pigpioCommand(
        reqParam({ command: 'SLRO', gpio: user_gpio, baud, extension })
      )
    }
    ...
    bb_serial_invert(user_gpio: number, invert: 0 | 1): Promise<number> {
      return pigpioCommand(
        reqParam({ command: 'SLRI', gpio: user_gpio, invert })
      )
    }
    ...
    spi_open(
      ...
      return pigpioCommand(
        reqParam({ command: 'SPIO', channel: spi_channel, baud, extension })
      )
    }
    ...
    file_seek(
      ...
      return pigpioCommand(
        reqParam({ command: 'FS', handle, offset: seek_offset, extension })
      )
    }
    ...
    wave_add_serial(
      ...
      return pigpioCommand(
        reqParam({ command: 'WVAS', gpio: user_gpio, baud, extension })
      )
    }
