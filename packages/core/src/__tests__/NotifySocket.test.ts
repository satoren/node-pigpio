...
const mockRequest = {
  ...
  request: jest.fn(
    (arg) => new Promise((resolve) => resolve({ ...arg, res: 0 }))
  ),
  close: jest.fn(),
...
test('notify edge', async () => {
  const e1: EdgeCallback = { gpio: 5, edge: 2, bit: 1 << 5, func: jest.fn() }
  const fallingEdge2: EdgeCallback = {
    gpio: 2,
    edge: 1,
    bit: 1 << 2,
    func: jest.fn(),
  }
  const risingEdge2: EdgeCallback = {
    gpio: 2,
    edge: 0,
    bit: 1 << 2,
    func: jest.fn(),
  ...
  mockSocket.emit(
    'data',
    notifyToBuffer({ seqno: 0, flags: 0, tick: 32, level: 1 << 5 })
  )
  ...
  mockSocket.emit(
    'data',
    notifyToBuffer({ seqno: 0, flags: 0, tick: 32, level: 0 })
  )
  ...
  mockSocket.emit(
    'data',
    notifyToBuffer({ seqno: 0, flags: 0, tick: 33, level: 1 << 2 })
  )
  expect(risingEdge2.func).lastCalledWith(2, 1, 33)
  expect(fallingEdge2.func).not.toBeCalled()
  mockSocket.emit(
    'data',
    notifyToBuffer({ seqno: 0, flags: 0, tick: 33, level: 0 })
  )
  expect(risingEdge2.func).lastCalledWith(2, 1, 33)
...
test('notify wdog', () => {
  ...
  mockSocket.emit(
    'data',
    notifyToBuffer({ seqno: 0, flags: WDOGFlag | 5, tick: 32, level: 0 })
  )
  ...
  mockSocket.emit(
    'data',
    notifyToBuffer({ seqno: 0, flags: WDOGFlag | 2, tick: 33, level: 0 })
  )
  expect(e1.func).lastCalledWith(5, 'TIMEOUT', 32)
...
test('notify event', () => {
  ...
  mockSocket.emit(
    'data',
    notifyToBuffer({ seqno: 0, flags: EventFlag | 5, tick: 32, level: 333 })
  )
  expect(e1.func).lastCalledWith(5, 32)
  expect(e2.func).not.toBeCalled()
  mockSocket.emit(
    'data',
    notifyToBuffer({ seqno: 0, flags: EventFlag | 2, tick: 555, level: 44543 })
  )
  expect(e1.func).lastCalledWith(5, 32)
