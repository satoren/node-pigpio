...
describe('bb spi', () => {
  test('open and close', async () => {
    ...
    expect(mockRequestSocket.request).toBeCalledWith({
      cmd: RequestCommand.BSPIO.cmdNo,
      extension: extent,
      p1: cs,
      p2: 0,
    ...
    expect(mockRequestSocket.request).toBeCalledWith({
      cmd: RequestCommand.BSPIC.cmdNo,
      p1: cs,
      p2: 0,
      responseExtension: false,
    })
  ...
  test('xfer', async () => {
    ...
    mockRequestSocket.request.mockResolvedValueOnce({
      cmd: RequestCommand.BSPIX.cmdNo,
      p1: cs,
      p2: 0,
      res: responseData.length,
...
describe('gpio', () => {
  test('on EITHER_EDGE', () => {
    ...
    expect(mockNotifySocket.append).toBeCalledWith({
      bit: 8,
      edge: 2,
      gpio: 3,
  ...
  test('on falling edge', () => {
    ...
    // invoke callback
    const func = mockNotifySocket.append.mock.calls[0][0].func as (
      gpio: number,
      level: 0 | 1 | 'TIMEOUT',
      tick: number
    ) => void
  ...
  test('write', async () => {
    await pi.write(3, 1)
    expect(mockRequestSocket.request).toBeCalledWith({
      cmd: RequestCommand.WRITE.cmdNo,
      p1: 3,
      p2: 1,
      responseExtension: false,
    })
  ...
  test.each([[1], [0]] as const)('setMode', async (mode) => {
    await pi.set_mode(3, mode)
    expect(mockRequestSocket.request).toBeCalledWith({
      cmd: RequestCommand.MODES.cmdNo,
      p1: 3,
      p2: mode,
      responseExtension: false,
    })
  ...
  test.each([[1], [0]] as const)('getMode', async (mode) => {
    mockRequestSocket.request.mockResolvedValueOnce({
      cmd: RequestCommand.MODEG.cmdNo,
      p1: 3,
      p2: 0,
      res: mode,
    })
  ...
  test.each([[0], [1], [2]] as const)('setPullUpDown %s', async (mode) => {
    mockRequestSocket.request.mockResolvedValueOnce({
      cmd: RequestCommand.PUD.cmdNo,
      p1: 3,
      p2: mode,
      res: 1,
    })
  ...
  test('setServoPulsewidth', async () => {
    mockRequestSocket.request.mockResolvedValueOnce({
      cmd: RequestCommand.SERVO.cmdNo,
      p1: 3,
      p2: 222,
      res: 1,
    })
...
describe('i2c', () => {
  ...
  test('write_device', async () => {
    ...
    expect(mockRequestSocket.request).toBeCalledWith({
      cmd: RequestCommand.I2CWD.cmdNo,
      p1: i2chandle,
      p2: 0,
      extension: writeData,
      responseExtension: false,
  ...
  test('read', async () => {
    ...
    const i2chandle = 99
    mockRequestSocket.request.mockResolvedValueOnce({
      cmd: RequestCommand.I2CRD.cmdNo,
      p1: i2chandle,
      p2: 0,
      res: responseData.length,
      extension: responseData,
    })
    const [s, readData] = await pi.i2c_read_device(
      i2chandle,
      responseData.length
    )
  ...
  test('zip', async () => {
    ...
    const i2chandle = 99
    mockRequestSocket.request.mockResolvedValueOnce({
      cmd: RequestCommand.BI2CZ.cmdNo,
      p1: i2chandle,
      p2: 0,
      res: responseData.length,
      extension: responseData,
...
describe('bb i2c', () => {
  ...
  test('open', async () => {
    ...
    expect(mockRequestSocket.request).toBeCalledWith({
      cmd: RequestCommand.BI2CO.cmdNo,
      extension: baud,
      p1: sda,
      p2: scl,
      responseExtension: false,
  ...
  test('close', async () => {
    ...
    expect(mockRequestSocket.request).toBeCalledWith({
      cmd: RequestCommand.BI2CC.cmdNo,
      p1: sda,
      p2: 0,
      responseExtension: false,
    })
