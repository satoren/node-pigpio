...
import {
  MonoTypedEventTarget,
  MonoTypedEventEmitter,
  TypedEventTarget,
...
function isSpiOption(option: unknown): option is SpiOption {
  return (
    (option as SpiOption).channel != null &&
    (option as SpiOption).baudRate != null
  )
}
...
class PigpioImpl implements Pigpio {
  ...
  constructor(pi: llpigpio.pigpio, autoClose: boolean) {
    this.pi = pi
  ...
  async i2c(option: I2COption | BBI2COption): Promise<I2c> {
    if (isI2COption(option)) {
      ...
      return this.autoCloseWrap(
        await this.i2cFactory.create(bus, address, flags)
      )
    }
    const { address, sda, scl, baudRate } = option
    return this.autoCloseWrap(
      await this.bbI2cFactory.create(address, sda, scl, baudRate)
    )
  }
  ...
  async spi(option: SpiOption | BBSpiOption): Promise<Spi> {
    if (isSpiOption(option)) {
      ...
      return this.autoCloseWrap(
        await this.spiFactory.create(channel, baudRate, flags)
      )
    }
    const { cs, miso, mosi, sclk, baudRate, flags = 0 } = option
    return this.autoCloseWrap(
      await this.bbSpiFactory.create(cs, miso, mosi, sclk, baudRate, flags)
    )
  }
  ...
  autoCloseWrap<
    T extends { closeEvent: MonoTypedEventTarget<void> } & Closable
  >(c: T): T {
    this.users.add(c)
