...
test('on falling edge', () => {
    ...
    const func = mockedCallback.mock.calls[0][2] as (gpio: number, level: 0 | 1 | 'TIMEOUT', tick: number) => void
    func(2, 1, 3)
...
test('once edge', () => {
    ...
    // invoke callback
    const func = asMock(mockedPigpio.callback).mock.calls[0][2] as (gpio: number, level: 0 | 1 | 'TIMEOUT', tick: number) => void
    func(2, 1, 3)
    expect(listener).toHaveBeenCalledTimes(1)
