...
test('write', async () => {
    ...
    expect(mockedPigpio.bb_i2c_zip).toBeCalledWith(sda, Buffer.of(4, device, 2, 7, 1, 21, 3, 0))
})
...
test('write over 255', async () => {
    ...
    mockedZip.mockResolvedValueOnce([responseData.length, responseData])
    await i2c.writeDevice(writeData)
    expect(mockedPigpio.bb_i2c_zip).toBeCalledWith(sda, Buffer.of(4, device, 2, 1, 7, 255, 3, ...writeData, 3, 0))
})
...
test('read', async () => {
    ...
    expect(mockedPigpio.bb_i2c_zip).toBeCalledWith(sda, Buffer.of(4, device, 2, 6, responseData.length, 3, 0))
...
test('read 2 sequence by zip ', async () => {
    ...
    const mockedZip = (mockedPigpio.bb_i2c_zip as jest.Mock)
    mockedZip.mockResolvedValueOnce([responseData.length + responseData2.length, Buffer.concat([responseData, responseData2])])
    const [readData, readData2] = await i2c.zip({ type: 'Read', size: responseData.length }, { type: 'Read', size: responseData2.length })
    expect(mockedPigpio.bb_i2c_zip).toBeCalledWith(sda, Buffer.of(4, device, 2, 6, responseData.length, 2, 6, responseData2.length, 3, 0))
    expect(readData).toMatchObject(responseData)
