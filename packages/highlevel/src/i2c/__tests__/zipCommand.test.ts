...
import { buildZipCommand, I2cZipCommand } from '@node-pigpio/highlevel/src/i2c/zipCommand'
...
test('build write command', () => {
    const command = buildZipCommand({ type: 'Write', data: Buffer.alloc(64) }, false)
...
test('build write command larger then 255', () => {
    const command = buildZipCommand({ type: 'Write', data: Buffer.alloc(500) }, false)
...
test('build read command larger then 65535', () => {
    expect(() => { buildZipCommand({ type: 'Read', size: 65536 }, false) }).toThrow()
})

test('invalid command', () => {
    expect(() => { buildZipCommand({ type: 'a' } as unknown as I2cZipCommand, false) }).toThrow()
})
