...
const createImageBuffer = () => {
    ...
    // sample animation from https://developer.mozilla.org/ja/docs/Web/Guide/HTML/Canvas_tutorial/Basic_animations
    const now = new Date()
    ctx.save()
    ctx.scale(0.4, 0.4)
...
(async () => {
    ...
    process.once('SIGINT', () => { void ssd1331.close() })

    while (true) {
        const image = createImageBuffer()
