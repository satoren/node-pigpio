...
const errorCodes = Object.entries(errorNames).reduce<
Record<number, ErrorNames>
>((ret, entry) => {
    const [key, value] = entry
    return { ...ret, [value]: key as ErrorNames }
}, {})
// PI_PIGIF_ERR_0    -2000
...
export function pigpioErrorMessage (res: number): string | undefined {
    return errorMessages[errorCodes[res]]
}

export class PigpioError extends Error {
    constructor (message: string | undefined, readonly errorcode: number) {
        super(message)
    }
}
export default { pigpioErrorMessage }
